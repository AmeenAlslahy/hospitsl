...
try {
    // المواعيد القادمة
    $upcoming_stmt = $db->preparedQuery("SELECT COUNT(*) as count FROM appointments 
                                 WHERE patient_id = ? 
                                 AND appointment_date >= CURDATE() 
                                 AND status = 'confirmed'", [$patient_id]);
    ...
    //                      WHERE pt.patient_id = ?
    $appointments_stmt = $db->preparedQuery("SELECT a.*,                          
                         du.full_name as doctor_name,
...
<div class="patient-dashboard py-4">
    <div class="container">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="mb-0">مرحباً بك، <?php echo htmlspecialchars($_SESSION['full_name'] ?? 'المريض'); ?></h1>
