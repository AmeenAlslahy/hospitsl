...
function getPatientId($user_id, $db) {
    $stmt = $db->preparedQuery("SELECT patient_id FROM patients WHERE user_id = ?", [$user_id]);
...
function checkPhoneAndEmail($username,$email,$db){
   $stmt = $db->preparedQuery("SELECT user_id FROM users WHERE username = ? OR email = ? LIMIT 1", [$username, $email]);
...
function getDoctorById($doctor_id, $db) {
    $stmt = $db->preparedQuery("SELECT d.*, u.full_name, u.email, u.phone 
                         FROM doctors d
...
function getDoctorId($user_id,$db) {
    $stmt = $db->preparedQuery("SELECT doctor_id FROM doctors WHERE user_id = ?", [$user_id]);
    $result = $stmt->get_result();
    if ($result->num_rows > 0) {
